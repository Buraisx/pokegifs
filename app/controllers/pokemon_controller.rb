class PokemonController < ApplicationController
	...
	def team
		6.times do
			pokemon = Random.new
			res  = HTTParty.get("http://pokeapi.co/api/v2/pokemon/#{pokemon.rand(600)}")
			body = JSON.parse(res.body)
			name = body["name"]
			id = body["id"]
			type = body["types"][0]["type"]["name"]
			...
			res = HTTParty.get("https://api.giphy.com/v1/gifs/search?api_key=aHtC6smrcaWbfwhKHkW5m1pJ0e1Ae4Ke&q=#{pokemon.rand(600)}&rating=g")
			body = JSON.parse(res.body)
			gif_url = body["data"][0]["url"]
